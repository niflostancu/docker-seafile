...
Adapted from the original docker image at
...
from utils import (
    ...
    get_seafile_version, read_version_stamp, wait_for_mysql,
    update_version_stamp, loginfo
)
...
def fix_media_symlinks(current_version):
    ...
    if not islink(avatars_dir):
        logger.info('Running minor-upgrade.sh to fix the media symlinks')
...
def fix_custom_dir():
    real_custom_dir = '/var/lib/seafile/seahub-data/custom'
...
def check_upgrade():
    ...
    if installed_version == current_version:
        ...
        return
    elif is_minor_upgrade(installed_version, current_version):
        run_minor_upgrade(current_version)
        return
